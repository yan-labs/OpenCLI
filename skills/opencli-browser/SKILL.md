...
- `--window dedicated|background|active|foreground|isolated` (or `OPENCLI_WINDOW=...`) chooses the window/tab policy for owned sessions: `dedicated` (default) keeps the session's tab only inside an OpenCLI-owned automation window (a *slot*) in this same Chrome, same profile — the window is created unfocused and never raised, but auto-select (on by default) makes the tab the window's active tab before every page-scoped command, so it renders `visible` without ever stealing OS focus; `background` instead borrows whichever window you are currently using and drops the tab into a labelled group there, following you if you switch windows (never raises or selects, so `visibilityState` stays `hidden`); `active` selects the tab within its own window only, without stealing macOS-level window focus (use this to keep a tab's own render loop, e.g. `requestAnimationFrame`, from being throttled while staying invisible to the user); `foreground` raises the window to real OS focus (interrupts the user; use only when a human needs to see it, or for the confirmed sites that only render frontmost — PageSpeed Insights, Google Trends chart rendering, AITDK's cross-origin iframe panel); `isolated` is `background` in its own separate window. `dedicated` windows are pooled — anonymous `pool-1`, `pool-2`, … windows are borrowed for a lease and returned to the pool when it ends, so a run of one-shot commands reuses one window instead of piling one up per session — tiled dynamically on the automation display so windows never overlap (Chrome reports an overlapped window as `hidden`), capped by display space (a command that would overlap fails with `dedicated-pool-exhausted: …` instead of stacking an unseeable window), and reaped after an idle TTL (default 15 min, override with `OPENCLI_DEDICATED_IDLE_MS`). With no explicit display, the extension auto-picks a secondary/non-internal display when one exists. Pin a stable window instead of drawing from the pool with `--window-slot <name>`, place it with `--window-bounds <x,y,w,h>` or `--window-display <pattern>`, and use `opencli browser <session> window status` / `window list` / `window ensure` / `window close [--slot <name>] [--force]` to inspect or manage dedicated windows independent of any session.
