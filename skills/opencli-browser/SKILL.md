...
- `--window foreground|active|background|isolated|dedicated` (or `OPENCLI_WINDOW=...`) chooses the window/tab policy for owned sessions: `background` (default) never raises or selects; `active` selects the tab within its own window only, without stealing macOS-level window focus (use this to keep a tab's own render loop, e.g. `requestAnimationFrame`, from being throttled while staying invisible to the user); `foreground` also raises the window to the OS foreground (interrupts the user; use only when a human needs to see it); `isolated` is background in its own separate window; `dedicated` keeps the session's tab only inside a named automation window (a *slot*) in this same Chrome, same profile — the window is created unfocused and never raised, but auto-select (on by default) makes the tab the window's active tab before every page-scoped command, so it renders `visible` without ever stealing OS focus. Pick the slot with `--window-slot <name>` (default `default`; use a different slot per session that needs concurrent visibility), place the window with `--window-bounds <x,y,w,h>` or `--window-display <pattern>`, and use `opencli browser window status` / `opencli browser window ensure` to inspect or set up dedicated windows independent of any session.
