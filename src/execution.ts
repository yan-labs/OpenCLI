...
function normalizeWindowMode(name: string, raw: unknown): BrowserWindowMode | null {
  if (raw === undefined || raw === '') return null;
  // Keep this list in sync with BrowserWindowMode, with getBrowserWindowMode() in
  ...
  if (raw === 'foreground' || raw === 'active' || raw === 'background' || raw === 'isolated' || raw === 'dedicated') return raw;
